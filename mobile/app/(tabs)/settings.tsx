  ...
  if (user?.permissions !== 'all') {
    return (
      <View style={[styles.container, { backgroundColor: background }]}>
        <Text style={[styles.unauthorized, { color: textSecondary }]}>Admin access required to view Settings.</Text>
      ...
      <View style={styles.logList}>
        {entries.slice(0, 300).map((e) => (
          <View key={e.id} style={styles.logRow}>
            <Text style={styles.logTime}>{formatDate(e.created_at)}</Text>
        ...
        {entries.length === 0 && <Text style={styles.muted}>No log entries.</Text>}
      </View>
    </View>
  );
}
  ...
  return (
    <View style={styles.panelInner}>
      ...
      {list.map((c) => (
        <View key={c.id} style={styles.listItem}>
          <View style={styles.listItemInfo}>
            ...
            <Text variant="bodySmall" style={styles.muted}>{c.amount} UGX / {c.freq}{c.note ? ` · ${c.note}` : ''}</Text>
          </View>
      ...
      <View style={styles.totalBar}>
        <Text variant="titleSmall">Monthly Total: {monthlyTotal.toLocaleString()} UGX | Break-Even: {breakEven.toLocaleString()} UGX/day</Text>
      </View>
      <FixedCostModal
        visible={modal.open}
        item={modal.item}
      ...
      {list.map((r) => (
        <View key={r.id} style={styles.listItem}>
          <View style={styles.listItemInfo}>
            ...
            <Text variant="bodySmall" style={styles.muted}>{r.inputType === 'count' ? `count | ${r.price ?? 0} UGX/unit` : 'direct amount'}</Text>
          </View>
          <View style={styles.listItemActions}>
      ...
      {list.map((p) => (
        <View key={p.id} style={styles.listItem}>
          <View style={styles.listItemInfo}>
            ...
            <Text variant="bodySmall" style={styles.muted}>{p.price} UGX | {p.duration} {p.durUnit}</Text>
          </View>
          <View style={styles.listItemActions}>
      ...
      {users.map((u) => (
        <View key={u.id} style={styles.listItem}>
          <View style={styles.listItemInfo}>
            ...
            <Text variant="bodySmall" style={styles.muted}>ID: {u.id} | {permSummary(u.permissions)} | {u.phone || 'No phone'}</Text>
          </View>
          <View style={styles.listItemActions}>
...
const styles = StyleSheet.create({
  ...
  listItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#334155' },
  listItemInfo: { flex: 1 },
  ...
  modalBox: { backgroundColor: '#1e293b', borderRadius: 12, padding: 20 },
